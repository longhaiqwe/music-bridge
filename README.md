...
下载链路现在只使用 `--cookies-from-browser`。默认浏览器是 `chrome`，也可以通过 `YOUTUBE_COOKIES_BROWSER` 指向具体 profile，例如 `chrome:Profile 2`。
...
`youtube:pot:doctor` 以及应用里的 YouTube 下载链路现在都会自动拉起本地 `bgutil` server，所以日常通常不需要手动先执行 `youtube:pot:server`；它更适合调试时单独观察日志。
